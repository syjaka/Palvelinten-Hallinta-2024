...
3. Varaston [terokarvinen/suolax/](https://github.com/terokarvinen/suolax/) historia - 11.4 klo 12.45 - 13.30 EET
  ...
  5. Tehtävänannossa ei ollut tarkennusta, mitä tällä historialla olisi tarkoitus tehdä. Päätin analysoida muutaman tapahtuman:
    ![h3-002](https://github.com/syjaka/Palvelinten-Hallinta-2024/blob/main/images/h3-002.png)
...
Kaikki seuraavien osioidenn tehtävänannot ovat peräisin Tero Karvisen - Infra As a Code - [Palvelinten hallinta 2024 kurssisivulta](https://terokarvinen.com/2024/configuration-management-2024-spring/#h3-toimiva-versio).
...
9. Lokia tarkastellessa tämä selittyi. Virtuaalikoneeni oli vielä eilisessä.
...
1. Aloitin määrittämällä koneen:
   - Siirryin emokoneellani kansioon `cd twohost` ja otin yhteyden siihen `vagrant  ssh k002`
...
![h3-013](https://github.com/syjaka/Palvelinten-Hallinta-2024/blob/main/images/h3-013.png)
   - `git add . && git commit; git pull && git push` komento palautti ao. virheen eli lisään aluksi nimen ja meilin.
   ...
   - Annoin komennot `git config --global user.email "kadriye.syrja@myy.haaga-helia.fi"` ja `git config --global user.name "Kadi Syrjä @k001"` ja kokeilin  uudelleen `git add . && git commit; git pull && git push` joka onnistuneesti päästi tallentamaan commitin.
     ![03-014](https://github.com/syjaka/Palvelinten-Hallinta-2024/blob/main/images/h3-014.png)
   - Lopputuloksena myös Githubissa näkyy luotu moduuli. (Väärä aikaleima harmitti, joten päivitin sen `sudo apt-get -y install ntp`ja `sudo systemctl enable --now ntp` 
...
3. Seuraavaksi tein *lempparit* moduulin, joka ajaa kaikki suosikkiohjelmat uutta konetta käyttöönottaessa `mkdir lempparit`
    ...
    - `git add . && git commit; git pull && git push` synkkasi muutokset Githubiin, josta vielä tarkistin että ne siirtyivät onnistuneesti.
...
6. Tein vielä topfilen, jolle voi antaa eri konetyyppejä, joille taas voi asentaa eri moduuleja.
...
7. Vielä viimeiseksi `git add . && git commit; git pull && git push`
...
1. Koneen autentikointi Githubiin ja edellisen *summer* repon kloonaaminen paikallisesti koneeseen.
   ...
   - Loin vielä uuden kansion ja tiedoston ja synkkasin ne Githubiin.
